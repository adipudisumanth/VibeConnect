  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] gap-4">
        ...
        <p className="text-muted-foreground animate-pulse">Gathering the latest vibes...</p>
      </div>
    );
  }
  ...
  if (isError) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] text-center p-6 border-2 border-dashed rounded-xl">
        ...
        <p className="text-muted-foreground mb-4">The signal is weak. Try refreshing the page.</p>
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="flex flex-col gap-2">
        ...
        <p className="text-muted-foreground text-lg max-w-2xl">
          Connect with founders and fellow VibeCoders to bring digital visions to life.
        </p>
      </div>
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        {projects?.map((project) => (
          ...
          >
            <CardHeader className="pb-3">
              <div className="flex items-center justify-between mb-3">
                <Badge variant="outline" className="bg-primary/5 border-primary/20 text-primary capitalize">
              ...
              <CardDescription className="text-sm">
                Openings: <span className="font-bold text-foreground">{project.filledOpenings}/{project.totalOpenings}</span>
              </CardDescription>
            </CardHeader>
            ...
            <CardFooter className="pt-4 border-t border-border/30">
              {isVibeCoder ? (
                <Button className="w-full h-11 font-bold shadow-md shadow-primary/10 group cursor-pointer" variant="default">
                  Apply Now
                  <Briefcase className="ml-2 h-4 w-4 transition-transform group-hover:rotate-12" />
                </Button>
              ) : (
                <Button className="w-full h-11 font-bold group cursor-pointer" variant="outline">
                  Manage Vibe
                  <ChevronRight className="ml-2 h-4 w-4 transition-transform group-hover:translate-x-1" />
                </Button>
              )}
